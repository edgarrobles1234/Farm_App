...
import { followUser, listFollowers, unfollowUser, type SearchUser } from "@/lib/follows";
...
export default function FollowersScreen() {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      try {
        const results = await listFollowers(accessToken, normalizedQuery, 200);
        if (!isActive) return;
        setUsers(results);
      } catch (e) {
        if (!isActive) return;
        const message = e instanceof Error ? e.message : "Unable to load followers";
  ...
  const toggleFollow = async (targetUserId: string) => {
    ...
    try {
      if (currentlyFollowing) {
        ...
        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
        setUsers((prev) => prev.map((u) => (u.id === targetUserId ? { ...u, is_following: false } : u)));
      } else {
            ...
            <View style={styles.userItem}>
              ...
              <TouchableOpacity
                ...
                style={[
                  styles.followButton,
                  {
                    backgroundColor: item.is_following ? theme.brand.darkerOrange : theme.brand.primary,
                  },
                ]}
              >
                <Ionicons name={item.is_following ? "checkmark" : "person-add"} size={21} color={theme.neutral.white} />
              </TouchableOpacity>
            </View>
          )}
